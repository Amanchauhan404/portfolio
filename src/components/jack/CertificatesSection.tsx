...
function CertificateCard({ cert, i, viewingIdx, setViewingIdx }: { cert: typeof CERTIFICATES[0]; i: number; viewingIdx: number | null; setViewingIdx: (v: number | null) => void }) {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { margin: "-40% 0px -40% 0px" });

  return (
    <FadeIn delay={i * 0.12} y={30}>
      <div className="group" ref={ref}>
        ...
        >
          ...
          <div className="flex flex-col gap-3 sm:gap-4 pt-1 sm:pt-2 min-w-0 flex-1">
            ...
            >
              {cert.issuer} &middot; {cert.date} &middot; Code: {cert.code}
            </p>
            ...
            <div className="flex flex-wrap gap-3 mt-3">
              ...
              >
                ...
                <svg
                  ...
                  style={{
                    transform: viewingIdx === i ? "rotate(90deg)" : "rotate(0deg)",
    ...
    >
      ...
      <div className="max-w-5xl mx-auto">
        {CERTIFICATES.map((cert, i) => (
          <CertificateCard key={cert.number} cert={cert} i={i} viewingIdx={viewingIdx} setViewingIdx={setViewingIdx} />
        ))}
      </div>
    </section>
  );
